CREATE PROCEDURE [Denormalised].[pGet_vIP_EncounterDenormalised] @SourceSchemaName [VARCHAR] (30)
...
BEGIN
	...
	SELECT [T].*
	FROM [Denormalised].[Materialised_vIP_EncounterDenormalised] AS [T]
	WHERE EXISTS (
			SELECT NULL
			FROM [Denormalised].[Materialised_Casted_IP_Attribution] AS [R]
