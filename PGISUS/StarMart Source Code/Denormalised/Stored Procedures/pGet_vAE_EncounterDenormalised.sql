CREATE PROCEDURE [Denormalised].[pGet_vAE_EncounterDenormalised] @SourceSchemaName [VARCHAR] (30)
...
BEGIN
	...
	SELECT [T].*
	FROM [Denormalised].[Materialised_vAE_EncounterDenormalised] AS [T]
	WHERE EXISTS (
			SELECT NULL
			FROM [Denormalised].[Materialised_Casted_AE_Attribution] AS [R]
