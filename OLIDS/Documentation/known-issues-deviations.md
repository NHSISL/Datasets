...
| KI06 | Patients will only ever match to PDS responses that share the same NHS Number. | The `PATIENT_PERSON` model uses the same method used in `PERSON` to generate the expected mapped `PERSON_ID`. Where a PDS trace results in a `MATCHED_NHS_NO` that differs to the requesting EMIS records NHS number, the resulting `PERSON.ID` value will differ. | Users may see a higher than normal number of EMIS based 'stub' person records, rather than person records based on PDS responses. | ![Status=Open](https://img.shields.io/badge/Status-Open-red) <br>[![DevOps](https://img.shields.io/badge/DevOps-29792-blue)](https://dev.azure.com/NELAnalytics/LondonDataService/_workitems/edit/29792) | ![Team=LDSTeam](https://img.shields.io/badge/LDS_Team-00ADC3) |
