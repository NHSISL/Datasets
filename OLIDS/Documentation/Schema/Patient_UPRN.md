...
| `STATUS` | `VARCHAR` | (Success/Null) Whether the attempt to match the uprn was a success (does not indicate whether the match to a uprn was a success, only the attempt) | | | |
| `MATCHED` | `VARCHAR` | (True/False) Whether a UPRN match was found | | | |
| `UPRN` | `VARCHAR` | The unique property reference number | | #️⃣ hashed | |
| `POSTCODE_QUALITY` | `VARCHAR` | The quality of the postcode within the supplied input address | | | |
| `QUALIFIER` | `VARCHAR` | AddressBase Premiums encoding for the type of matched address (residential, child)  | | | |
| `CLASSIFICATION` | `VARCHAR` | AddressBase Premium encoding for the property classification<sup>1</sup> | | | |
| `ALGORITHM` | `VARCHAR` | the algorithm used to achieve the best address match | | | |
| `MATCH_PATTERN` | `VARCHAR` | the matched pattern structure | | | |
